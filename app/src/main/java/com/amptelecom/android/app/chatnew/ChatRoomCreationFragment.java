...
import com.amptelecom.android.app.contacts.SearchContactViewHolder;
import com.amptelecom.android.app.contacts.SearchContactsAdapter;
import com.amptelecom.android.app.contacts.views.ContactSelectView;
...
public class ChatRoomCreationFragment extends Fragment
    ...
    private ArrayList<String> listNumbers = new ArrayList<>();

    @Override
    public View onCreateView(
            LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        View view = inflater.inflate(R.layout.chat_create_new, container, false);
        ...
        mNextButton.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        //                        if (mChatRoomAddress == null && mChatRoomSubject
                        // == null) {
                        //                            mContactsSelectedLayout.removeAllViews();
                        //                        } else {
                        //                            // Pop the back stack twice so we don't have
                        // in stack
    ...
    public void onItemClicked(int position) {
        ...
        if (!mCreateGroupChatRoom) {
            ...
            addOrRemoveContactFromSelection(
                    new ContactAddress(c, "", searchResult.getPhoneNumber()));
        }
    }
        ...
        call.enqueue(
                new Callback<GeneralResponse>() {
                    ...
                    public void onResponse(
                            Call<GeneralResponse> call, Response<GeneralResponse> response) {
                        android.util.Log.i("ttt", "success" + response.body().getData());
    ...
    public void onItemClickedOld(int position) {
        SearchResult searchResult = mSearchAdapter.getContacts().get(position);
    ...
    private void addSelectedContactAddress(ContactAddress ca) {
        ...
        listViews.add(viewContact);
        mContactsSelectedLayout.addView(viewContact);
        mContactsSelectedLayout.invalidate();
    }

    private void updateContactsClick(ContactAddress ca) {
        boolean isSelected = listNumbers.contains(ca.getPhoneNumber());
        if (!isSelected) {
            listNumbers.add(ca.getPhoneNumber());
            ContactSelectView csv = new ContactSelectView(getActivity());
            csv.setListener(this);
            csv.setContactName(ca);
            addSelectedContactAddress(ca);
        } else {
            int pos = listNumbers.indexOf(ca.getPhoneNumber());
            android.util.Log.i(
                    "ttt", "p" + pos + "s" + listNumbers.size() + "sv" + listViews.size());
            listNumbers.remove(pos);
            View vv = listViews.get(pos);
            listViews.remove(pos);
            try {
                mContactsSelectedLayout.removeView(vv);
                mContactsSelectedLayout.invalidate();
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            //
            //            mContactsSelectedLayout.removeAllViews();
            //            for (ContactAddress contactAddress :
            // mSearchAdapter.getContactsSelectedList()) {
